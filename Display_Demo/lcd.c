...
void LCD_printLine(uint16_t row, uint16_t col, char* line, int len){
  ...
  assert((col + len)*LCD_GetFont()->Width <= 320);
...
void LCD_ClearLine(uint16_t Line){
  int i;
  for (i=320; i*LCD_GetFont()->Width >= 0; i-=LCD_GetFont()->Width)
  {
       LCD_DisplayChar(Line, i, ' ');
  }
}

void LCD_printHex(uint16_t row, uint16_t col, int h){
  char buffer [50];  
  int n;
  n=sprintf (buffer, "%#04x", h);
  LCD_printLine(row, col, buffer, n);  
}

void LCD_printNum(uint16_t row, uint16_t col, int h){
  char buffer [50];  
  int n;
